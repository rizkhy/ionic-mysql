...
export class JadwalSiswaPage {
   ...
   getData(){
    var dataa = localStorage.getItem('userData');
      ...
      let data = JSON.stringify(dataa);
  		...
  		this.http.post('http://localhost/siakad_phpV3/api/jadwal-siswa.php', data, {headers: headers})
      .map(res => res.json())
      .subscribe(data =>{
        this.items = data;        
      })
  ...
  getHari(){
    ...
    var harii = this.hari;
      ...
      console.log(a)
  		this.http.post('http://localhost/siakad_phpV3/api/jadwal-hari.php', a, {headers: headers})
      .map(res => res.json())
      .subscribe(data =>{
        this.items = data;        
      })
